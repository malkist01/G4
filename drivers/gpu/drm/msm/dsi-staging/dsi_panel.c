	...
	if (strstr(g_lcd_id, "huaxing") != NULL) {
		...
		for (i = 0; i < r_config->count; i++) {
			...
			if (r_config->sequence[i].sleep_ms)
				usleep_range(r_config->sequence[i].sleep_ms *
					     1000,
					     (r_config->sequence[i].sleep_ms *
					      1000) + 100);
		}
	}
#else
...
int dsi_panel_set_brightness(struct dsi_panel *panel, u8 dimming,
			     u32 brightness)
{
	int rc = 0;
	...
	((u8 *)panel->cur_mode->priv_info->
		...
		(brightness >> 8) & 0xf;
	((u8 *)panel->cur_mode->priv_info->
		cmd_sets[DSI_CMD_SET_BACKLIGHT].cmds[0].msg.tx_buf)[2] =
		brightness & 0xff;
...
int dsi_panel_set_dimming_brightness(struct dsi_panel *panel, u8 dimming,
				     u32 brightness)
{
	int rc = 0;
	...
	((u8 *)panel->cur_mode->priv_info->
		cmd_sets[DSI_CMD_SET_DIMMING_DBV].cmds[0].msg.tx_buf)[1] =
		dimming;
	((u8 *)panel->cur_mode->priv_info->
		cmd_sets[DSI_CMD_SET_DIMMING_DBV].cmds[1].msg.tx_buf)[1] =
		(brightness >> 8) & 0xf;
	((u8 *)panel->cur_mode->priv_info->
		cmd_sets[DSI_CMD_SET_DIMMING_DBV].cmds[1].msg.tx_buf)[2] =
		brightness & 0xff;
