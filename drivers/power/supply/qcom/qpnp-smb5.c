...
#ifdef CONFIG_MACH_XIAOMI_C3J
	rc = smblib_masked_write(chg, USBIN_9V_AICL_THRESHOLD_REG,
				 USBIN_9V_AICL_MASK,
				 USBIN_9V_AICL_THRESHOLD_CFG);
	if (rc < 0) {
...
{
	struct smb_charger *chg =
		container_of(work, struct smb_charger, fb_notify_work);
	...
	if ((lct_backlight_off) && (LctIsInCall == 0)) {
		if (lct_therm_lvl_reserved.intval >= 2)
			/* level 2 2.2A */
			smblib_set_prop_system_temp_level(
				chg, &lct_therm_globe_level);
		else
			/* from thermal-level */
			smblib_set_prop_system_temp_level(
				chg, &lct_therm_lvl_reserved);
