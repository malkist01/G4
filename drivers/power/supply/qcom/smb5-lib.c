...
#ifdef CONFIG_MACH_XIAOMI_F9S
		/* UFP is detected as SDP in platform,
		 * set 900mA when UFP type is detected */
		rc = smblib_read(chg, TYPE_C_CURRSRC_SNK_SRC_REG, &reg_ufp_try);
		rc |= smblib_read(chg, TYPE_C_CURRSRC_LEGACY_CABLE_REG,
				&reg_ufp_legacy);
		if (rc < 0) {
			smblib_err(chg, "Couldn't get typeC cable info rc=%d\n",
				   rc);
		} else if ((reg_ufp_try & BIT(0)) &&
			   !(reg_ufp_legacy & BIT(1))) {
...
void smb5_set_calling_current(struct smb_charger *chg)
{
	...
	if (chg->call_state == 0) {
		if (!strcmp(apsd_result->name, "DCP")) {
			...
			vote(chg->usb_icl_votable, CALL_ICL_MAX_VOTER, true,
			     DCP_CALL_CURRENT);
		}
		if (!strcmp(apsd_result->name, "HVDCP3") ||
		    !strcmp(apsd_result->name, "HVDCP2")) {
...
void batt_temp_charging_current(struct smb_charger *chg, int bat_temp)
{
	...
	if (bat_current != chg->last_bat_current) {
		pr_err("bat_current = %d,bat_temp = %d,last_bat_current=%d\n",
		       bat_current, bat_temp, chg->last_bat_current);
		rc = vote(chg->fcc_votable, BATT_TEMP_VOTER, true,
			  bat_current);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't vote fastchg ma rc = %d\n",
				rc);
		}

		rc = vote(chg->chg_disable_votable, BATT_TEMP_VOTER,
			  (bat_current == 0) ? true : false, 0);
		if (rc < 0) {
			dev_err(chg->dev, "Couldn't set battchg suspend rc %d\n",
				rc);
		}
...
#ifdef CONFIG_MACH_XIAOMI_C3J
	if (stat & (SNK_RP_STD_DAM_BIT | SNK_RP_1P5_DAM_BIT |
	    SNK_RP_3P0_DAM_BIT)) {
		smblib_masked_write(chg, TYPE_C_DEBUG_ACC_SNK_CFG,
				    TYPEC_DEBUG_ACC_SNK_DIS_AICL, 0);
		smblib_masked_write(chg, TYPE_C_DEBUG_ACC_SNK_CFG,
				    TYPEC_DEBUG_ACC_SNK_DIS_AICL |
				    TYPEC_DEBUG_ENABLE_CHG_ON_SNK,
				    TYPEC_DEBUG_ENABLE_CHG_ON_SNK);
	...
	if (chg->real_charger_type == POWER_SUPPLY_TYPE_USB_FLOAT) {
		if (usb_current == -ETIMEDOUT) {
			...
			if (chg->connector_type ==
				 ...
				 * based of Rp.
				 */
#if defined (CONFIG_MACH_XIAOMI_F9S) || defined (CONFIG_MACH_XIAOMI_C3J)
				typec_mode = 0;
...
void smblib_usb_plugin_locked(struct smb_charger *chg)
{
	...
	if (vbus_rising) {
		...
		if (need_confirm && attach_time - detach_time <
		    DETACH_ATTACH_MAX_INTERVAL) {
			rc = smblib_masked_write(chg, USBIN_OPTIONS_1_CFG_REG,
						 HVDCP_EN_BIT, 0);
			if (rc < 0)
		...
		if (chg->collapsed) {
			...
			if (detach_time - chg->recent_collapse_time <
			    COLLAPSE_DETACH_MAX_INTERVAL &&
			    detach_time - attach_time <
			    ATTACH_DETACH_MAX_INTERVAL)
				need_confirm = true;
			chg->collapsed = false;
		}
#endif
...
static void usb_current_worker(struct work_struct *work)
{
	...
	if ((!strcmp(apsd_result->name, "SDP")) && (settled_ua <= 100000) &&
	    !suspend) {
	...
	if ((!strcmp(apsd_result->name, "FLOAT")) && (settled_ua <= 100000) &&
	    !suspend) {
...
static void period_update(struct work_struct *work)
{
	...
	static int old_temp = 0, old_cap = 0, old_status = 0, old_present = 0,
		   old_usb_present = 0, printk_counter = 0, usb_v = 0;
	...
	/* if heatbeat_ms is longer than 500ms, must output the logs directly. */
	if ((period_ms >= 500) || (old_cap != cap) || (old_status != status) ||
	    (old_present != batt_p) || (old_usb_present != usb_p) ||
	    (printk_counter >= 5)) {
		pr_err("***temp=%d,v_batt=%dmV,c_batt=%dmA,cap=%d,status=%d,charge_type=%d,batt_p=%d,usb_p=%d,usb_v=%dmV,chg_done=%d\n",
		       temp, v_batt, c_batt, cap, status, charge_type, batt_p,
		       usb_p, usb_v, chip->chg_done);
	...
	/* When temperature changes by 3 degrees, report power_supply_changed */
...
int smblib_init(struct smb_charger *chg)
{
	...
	INIT_DELAYED_WORK(&chg->usbov_delay_report_work,
					smblib_usbov_delay_report_work);
